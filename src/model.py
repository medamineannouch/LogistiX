...
def multiple_src(prods,weight, cust, dc, dc_ub, plnt, plnt_ub, demand, tp_cost, del_cost, dc_fc, dc_vc, dc_num):
    """
    ...
    for j in tqdm(dc):
        DC_UB_Cons[j] = model.addConstr(
            dc_ub[j] * y[j]
            >=
            quicksum(x[i, j, p] for i in plnt for p in prods if (i, j, p) in plnt_to_dc),
            name=f'DC_UB_Cons[{j}]'
        )
